...
export default class MsPacman {
  ...
  draw(ctx, pause, enemies, pacman) {
    if (!pause) {
      this.#move();
      this.#animate();
    }
    this.#eatDot();
    this.#eatPowerDot(pacman);
  ...
  #eatPowerDot(pacman) {
    if (this.tileMap.eatPowerDot(this.x, this.y)) {
      ...
      this.powerDotAboutToExpire = false;
      pacman.powerDotAboutToExpire = false;
      this.timers.forEach((timer) => clearTimeout(timer));
      pacman.timers.forEach((timer) => clearTimeout(timer));
      this.timer = [];
      pacman.timer = [];

      let powerDotTimer = setTimeout(() => {
        this.powerDotActive = false;
        pacman.powerDotActive = false;
      ...
      pacman.timers.push(powerDotTimer);

      let powerDotAboutToExpireTimer = setTimeout(() => {
        this.powerDotAboutToExpire = true;
        pacman.powerDotAboutToExpire = true;
      }, 1000 * 3);

      this.timers.push(powerDotAboutToExpireTimer);
      pacman.timers.push(powerDotAboutToExpireTimer);
    }
  }
}
